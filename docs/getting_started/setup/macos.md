...
4. Change the following project settings:
    ...
    === "Single Header"
        ...
        2. Add header search paths for asio's folder:
           a. `/usr/local/include`, and
           b. **Apple Silicon**: `/opt/homebrew/Cellar/asio/<asio_version>/include`
           c. **Apple Intel**: `/usr/local/Cellar/asio/<asio_version>/include`
        3. Add linker flags (`-lpthread` for g++, `-pthread` for clang++)

6. Write your Crow application in `main.cpp` (something like the Hello World example will work).
7. Press `▶` to compile and run your Crow application.
